...
const AttendanceTracker = () => {
  ...
  useEffect(() => {
    if (!isAttendanceActive || students.length === 0) {
      console.log('Face recognition simulation not active', { isAttendanceActive, studentsCount: students.length });
      return;
    }
    ...
    const simulateRecognition = () => {
      ...
      console.log('Unrecognized students:', unrecognizedStudents.length);
      ...
      console.log('Recognized student:', recognizedStudent.name);
    ...
    const scheduleNextRecognition = () => {
      ...
      console.log(`Scheduling next recognition in ${Math.round(delay)}ms`);
    ...
    return () => {
      console.log('Cleaning up face recognition simulation');
      clearTimeout(timer);
    };
  ...
  const handleStartAttendance = () => {
    ...
    setTimeout(() => {
      startAttendance();
      setIsLoading(false);
      setCurrentDateTime(new Date()); // Reset current time when starting
    }, 1500);
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          {isAttendanceActive && (
            <div className="flex flex-col gap-1 p-3 bg-slate-50 rounded-md mb-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xl font-bold text-slate-700">{formatTime(currentDateTime)}</span>
              </div>
            </div>
          )}
